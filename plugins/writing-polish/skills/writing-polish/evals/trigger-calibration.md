...
- [轴 C：P0 站位检查轴（v10.0 新增）](#轴-cp0-站位检查轴v100-新增)
...
## 轴 C：P0 站位检查轴（v10.0 新增）

前提：技能已触发。测 P0 站位四问是否在该问时问、不该拦时不拦。重点防两类失控：对所有稿件机械追问四问（包括用户已明说的）；对熟人 / 私人场景过度拦截（把日常客气当商务腔报错，或反过来把正常礼貌当错位）。

| # | 输入 + 稿件条件 | 期望 | 依据 |
|---|---|---|---|
| C1 | 润色一篇党课讲稿，用户未说受众与媒介 | fire：先问清四问（尤其媒介与第二受众：是否印发、抄送谁），答齐再动笔 | P0 全模式第一环节 |
| C2 | 润色一篇调研报告，用户已明说“给甲方项目负责人看，邮件发” | **不再追问**，直接进 P1；四问已有答案 | 已答不重问 |
| C3 | “帮我给妈妈写条微信，说周末回家” | Express；站位一行确认（家人朋友 + 微信）；**不得**写成商务腔，也不得因无敬语报错 | familiar.md + §2.0 假阳 |
| C4 | 润色一段发给熟人的微信消息，语体已很日常 | **不拦**；即使 reviewer 报“过于正式”也只提示不阻断 | §2.0 假阳：熟人场景只提示不阻断 |
| C5 | 润色一篇 G2 领导讲话稿 | 逐句听众反应预演开跑；reviewer 输出含听众预演核对 | §2.1 门控 G2 |
| C6 | 润色一篇随笔（G6）或自媒体文（G7） | **不跑**听众反应预演；声音匹配层正常开 | §2.0 假阳：预演只门控 G2 |
| C7 | 润色一段面向软件系统的提示语 / 说明文 | 按直陈、无寒暄、术语保真处理，不把无寒暄报为冷漠 | 矩阵行：面向软件系统 |
| C8 | Audit 模式扫一篇 300 字短稿，用户未交代受众且无从推断 | 一行式站位确认后**跳过**站位判断，只查字面红线 | Audit step 0 降级条款 |

> C1 / C5 属非确定性 LLM 行为（与轴 A 同类），以人工观察为主；C3 / C4 / C6 是防误伤用例，若实际行为与期望相反，优先回看 stance-and-register §2.0 假阳清单是否被遵守，而非改用例。
