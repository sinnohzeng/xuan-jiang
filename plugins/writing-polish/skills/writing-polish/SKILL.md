...
| 评价输出 | **自然语言可执行反馈与粗判闸门**（够好了/要改/红线未清） | 数值逐维分（offline benchmark） |
...
不只查“哪里写得像 AI”，还要判断“整体读起来像不像真人写的”。词面禁令改不掉深层结构，列出被禁词还会反向激活该词（白熊效应），因此负面清单精简至约 80 条，释放的认知带宽用于正向引导（结论来源见 CHANGELOG 与 `docs/research/`）。
...
| 阶段 | 对应步骤 | 职责 | 关键工具 |
|------|---------|------|----------|
| **P0 定站位** | 全模式第一环节 | 站位四问：读者是谁 / 我是谁 / 媒介与文体 / 第二受众 | [`references/stance-and-register.md`](references/stance-and-register.md) §1 + [`references/scenarios/`](references/scenarios/) |
...
**P0 不可省**：任何写作 / 润色前先核实“读者是谁、在什么媒介上读到或听到”。答不上来先问用户，不替用户假设；短稿可一行式确认（读者、媒介）。四问细则与场景包见 stance-and-register §1-§2。
...
|---|---|---|---|---|
| **Coach** | 帮我写 / 起草 / 拟稿 / 搭提纲 / 草稿 / draft / outline | 监督生成弧（立意、构思、提纲、材料、结构五段，逐段 checkpoint） | 提纲、段落范本 | 5-15 min |
...
| **Express** | 快速改 / 帮我顺一下 / 看看这段 / 这段改改 | P0 一行式站位确认，L1 scan，主对话正向检查，单轮改稿（无 reviewer） | 修改稿、1 行说明 | 30s-1min |

### 1.2 触发词歧义解析

歧义触发词“帮我看看”“改一改”：主对话先估字数（可用 `scripts/word-count-check.sh`）再给推荐与理由，30s 窗口可改：
...
文案示例：`“建议 Polish（800 字属标准档；将 spawn 1 个全焦点 reviewer）。30s 内回复 audit/coach 可改。”`。用户已在触发词明示 mode，则跳过该步。
...
0. **P0 定站位 + 三个吃透（排在判体裁之前，四问不可跳）**：
   ① **读者是谁**（不可跳）：什么单位、什么层级、什么专业背景、拿这份材料去干什么。“客户”不算答案，“协会信息技术部主任，处级，不懂技术细节，明天拿去向副秘书长汇报”才算。
   ② **我是谁**（不可跳）：什么机构、与读者是什么关系、有没有资格下判断、有没有资格提要求。
   ③ **媒介与文体**（不可跳）：会场口播听到 / 纸面印发读到 / 屏幕扫读 / 微信即时通讯 / 机器解析；听稿句子宜短、忌生僻词、重复是特性不是缺陷。
   ④ **第二受众**（不可跳）：讲话稿印发下去给谁读、抄送范围到哪里；按第一、第二受众中更严格的一头定口吻。
   → 依据见 [`references/stance-and-register.md`](references/stance-and-register.md) §1-§2；方法论出处：任仲然《怎样写作》第七讲“一要明白谁要讲话，二要明白在什么场合对什么人讲话，三要明白印发下去的讲话给谁看”
   ⑤ 吃透意图：这篇稿子要传达什么核心信息？
   ⑥ 吃透素材：有哪些现成材料可用？
   → ①-④ 的答案决定全篇口吻，**答不上来先问用户，不替用户假设**
...
### 2.2 Polish Protocol（5 步，主对话照此执行）

<!-- scan-skip -->
```
step 0 — P0 定站位
  核实站位四问（读者 / 我方 / 媒介 / 第二受众）；
  缺失先问用户，不替用户假设；答案供 step 2.2 注入 reviewer prompt
...
step 2 — clean-context reviewer(s)（按 references/reviewer-routing.md 分焦点）
  ...
  2.2 每个 reviewer 用 Task 工具 spawn writing-reviewer 子代理（clean context），任务 prompt 注入：
        draft 全文 + **§1 站位四问的答案（读者是谁 / 我是谁 / 媒介与文体 / 第二受众）** + 该 reviewer 的 focus 列表
...
<!-- scan-skip -->
```
step 0: P0 站位确认（一行式）：读者 + 媒介；用户未交代且无从推断时跳过站位判断，只查字面红线
...
- **tier-1（默认，不落盘）**：请用户贴 2-3 段旧作真迹，做六维观察（句长节奏/用词偏好/标点习惯/段落组织/修辞倾向/语气人称，只取反复出现的习惯），并入 Polish step 3 有序 sweep 作附加约束，不新增 reviewer。
...
## §4 五大审查焦点与红线（NL，reviewer 据此分组反馈）
...
**口吻与站位为什么排第一**：立意 / 结构 / 材料 / AI 味出问题是“不够好”，站位出问题是“不能用”。动笔前先答四问：**读者是谁**、**我是谁**、**媒介与文体**、**第二受众**（含听稿规则与媒介错位形态）。答不上来先问用户，不替用户假设。判依据全文见 [`references/stance-and-register.md`](references/stance-and-register.md)。

**事实敬畏三态**（材料焦点核心）：① 已有材料可证实，直接用；② 用户未提供需追问，列清单问用户，不替写；③ 不得替用户编造。
...
| `agents/writing-reviewer.md` | clean-context reviewer 子代理（只评不改，返回 NL 反馈与 verdict） |
...
**大类速判**：戒假大空，判要改；戒翻译腔，判要改；戒AI味，判红线未清
...
## §5 资源路由表（Mode 对应必读，progressive disclosure）
...
| **Polish** | `agents/writing-reviewer.md` + `references/reviewer-routing.md` + [`references/stance-and-register.md`](references/stance-and-register.md)（第一顺位焦点，**必读**） + [`references/revision-fidelity.md`](references/revision-fidelity.md)（step 3.1.5 忠实性自查，**必读**） | reviewer 判依据 → [`references/constitution.md`](references/constitution.md) 体裁切片；G2 高政治敏感体裁 → [`references/scenarios/party-lecture.md`](references/scenarios/party-lecture.md)；step 3 改稿 → [`references/revision-checklist.md`](references/revision-checklist.md)；结构与论据焦点 → [`references/logic-and-structure.md`](references/logic-and-structure.md)；声音匹配（G6/G7）→ [`references/voice-matching.md`](references/voice-matching.md)；翻译腔与正向语感（step 3 第 ⑤ 维）→ [`references/positive-chinese-guide.md`](references/positive-chinese-guide.md) |
| **Audit** | `scripts/scan-ai-taste.sh` | L1 fail → `references/anti-ai-taste-anchors.md` |
<!-- /scan-skip -->

完整资源路由见 [`references/resource-routing.md`](references/resource-routing.md)（按需加载）；任仲然 12 讲继承审计见 [`references/renzhongran-coverage-matrix.md`](references/renzhongran-coverage-matrix.md)。
...
- 先大后小（立意、结构、段落、字词），先减后加，可改可不改的不改，受众/批评者换位，念改（step 3 完整决策清单见 [`references/revision-checklist.md`](references/revision-checklist.md)）
